...
| | |
|---|---|
| Backend tests | 904 passing, 3 deselected (network-marked) |
| Frontend tests | 178 passing across 15 files |
| Tables | 20 |
...
| **Watch what the model costs** | ongoing | Spend went from $0.09 to $1.12 in one afternoon, almost all of it agent runs. A run is roughly $0.10--$0.15 with caching on, an Explain is $0.02 (cached against the file, so the second look is free), and a repository survey is $0.07. That is fine occasionally and not fine as a habit; the Spend page breaks it down by feature. |
